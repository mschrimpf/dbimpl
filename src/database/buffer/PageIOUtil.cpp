...
void PageIOUtil::readPage(uint64_t pageId, uint64_t segmentId, void *data, unsigned len) {
	...
	if (segmentInfo == nullptr) {
		debug("Create file %s", std::to_string(segmentId).c_str());
		fd = open(std::to_string(segmentId).c_str(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
		debug("File created %s", std::to_string(segmentId).c_str());
		this->segmentInfoMap[segmentId] = {fd};
	} else {
		fd = this->segmentInfoMap[segmentId].fd;
	}
	debug("Reading file %s", std::to_string(segmentId).c_str());
	this->readFd(fd, offset, data, len);
	debug("File read %s", std::to_string(segmentId).c_str());
}

void PageIOUtil::readFd(int fd, long offset_bytes, void *data, unsigned len) {
	size_t read_bytes = 0;
	while (read_bytes < len) {
		read_bytes += pread(fd, ((char *) data) + read_bytes, len - read_bytes, offset_bytes + read_bytes);
		debug("Read %d bytes", read_bytes);
		if(read_bytes == 0) {
...
void PageIOUtil::writeFd(int fd, long offset_bytes, void *data, unsigned int len) {
	...
	while (written_bytes < len) {
		written_bytes += pwrite(fd, (uint8_t *) data + written_bytes, len - written_bytes,
								offset_bytes + written_bytes);
	}
